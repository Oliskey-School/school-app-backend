...
const aiService = new ai_service_1.AIService(GEMINI_API_KEY);
...
const generateResponse = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const queryHash = crypto_1.default.createHash('sha256').update(question + JSON.stringify(options || {})).digest('hex');
        // Allow ignoring cache error if table doesn't exist
        const { data: cachedData, error: cacheError } = yield supabase
            .from('ai_cache')
            .select('*')
        ...
        if (keywords.length > 0) {
            ...
            if (docs && docs.length > 0) {
                contextSnippets = docs.map((d, i) => `${i + 1}) ${d.title}::${d.content.substring(0, 200)}...`).join('\n');
...
USER_PREFERENCES:
- image_needed: ${(options === null || options === void 0 ? void 0 : options.image_needed) || false}
`;
        // 5. Generate with AI
        const aiResponseText = yield aiService.generateContent(finalPrompt, SYSTEM_PROMPT);
        let aiResponseJson;
        try {
